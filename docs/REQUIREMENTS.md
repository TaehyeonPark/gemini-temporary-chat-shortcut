...
| 방식 | `Cmd+Shift+N` 재정의 |
|------|:-------------------:|
| Chrome Commands API | ❌ 불가 — 공식 문서에서 명시적 제한 |
| Content Script `keydown` + `preventDefault()` | ❌ 불가 — 이벤트 자체가 페이지에 전달되지 않음 |

따라서 Chrome과 충돌하지 않는 **`Option+Shift+N`** (`Alt+Shift+N`)을 채택.
...
> Gemini UI는 SPA이므로 DOM이 동적으로 로드된다. 버튼이 아직 렌더링되지 않았을 수 있으므로 polling 재시도 로직을 사용한다.
